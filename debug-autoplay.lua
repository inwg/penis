...
local function setupGUI()
    ...
    if not pg then dprint("PlayerGui not found"); return false end
    local rsUI = tryGet(function() return pg:FindFirstChild("RhythmServiceUI") end)
    if not rsUI then dprint("RhythmServiceUI not found"); return false end
    local root = tryGet(function() return rsUI:FindFirstChild("RhythmRoot") end)
    if not root then dprint("RhythmRoot not found"); return false end
    ...
    if not receptors or not lanesHost then dprint("Receptors or Lanes missing"); return false end
    ...
    for i = 1, 4 do
        ...
        if rec then
            local ap = tryGet(function() return rec.AbsolutePosition end)
            local as = tryGet(function() return rec.AbsoluteSize end)
            if ap and as then
                newReceptors[i] = {
                    inst = rec,
                    cx   = ap.X + (as.X / 2),
                ...
                dprint(string.format("Receptor%d pos: (%.1f, %.1f)", i, ap.X, ap.Y))
            end
        end
    end

    if validLanes == 0 then dprint("No valid receptors found"); return false end
    activeLaneCount = validLanes
    if activeLaneCount == 2 then
        currentKeys = { 0x46, 0x4A }
    else
        currentKeys = { 0x58, 0x43, 0x4E, 0x4D }
        activeLaneCount = 4   -- 确保与 currentKeys 长度一致
    end
    ...
    -- 检查是否所有受体 X 相同（异常情况）
    local firstCX = newReceptors[1] and newReceptors[1].cx
    local allSame = true
    for i = 2, validLanes do
        if newReceptors[i] and newReceptors[i].cx ~= firstCX then
            allSame = false; break
        end
    end
    if allSame then
        dprint("所有 X 坐标相同，可能 AbsolutePosition 失效，尝试硬编码位置")
        local viewSize = game:GetService("Workspace").CurrentCamera.ViewportSize
        local left = viewSize.X * 0.25
        local right = viewSize.X * 0.75
        for i = 1, validLanes do
            if newReceptors[i] then
                local ratio = (i - 1) / (validLanes - 1)
                newReceptors[i].cx = left + (right - left) * ratio
                dprint(string.format("硬编码 Lane%d cx=%.1f", i, newReceptors[i].cx))
            end
        end
    end
    ...
    if enabled and showESP then hitLine.Visible = true end

    dprint("setup complete. activeLaneCount=", activeLaneCount)
    return true
end
...
local TOUCH_DIST   = 30   -- 原 20
local PAST_CATCH   = 35   -- 原 25
